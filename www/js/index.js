...
$(document).off('click', '#submit').on('click', '#submit', function() { 
    ...
    if($('#username').val().length > 0 && $('#password').val().length > 0){
        ...
        $.ajax({url: 'http://staging.eimpressive.com/slim_force/slim_four_ripsssjs/index.php',
...
$(document).off('click', '#chngpass').on('click', '#chngpass', function() { 
  //alert('chngpass');
        ...
        console.log($('#check_userpass').serialize());
                    $.ajax({url: 'http://staging.eimpressive.com/slim_force/slim_four_ripsssjs/chngpass.php',
